...
class FileWriter:
	def __init__(self,filename):
		self.filename = filename #此属性用于记录文件名
	...
	def __enter__(self):
		...
		return self # 返回值用于with 中 as 绑定

	def __exit__(self,exec_type,exec_value,exec_tb):
		'''
		exec_type:exception type,没有异常发生时为None
		exec_value:exception value,错误对象
		...
		print('__exit__方法被调用，已经离开with语句')
