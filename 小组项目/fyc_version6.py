...
class Application_ui(Frame):
    ...
    def __init__(self, master=None, **kwargs):
        ...
        self.createMenu()
        self.createLabel()
        lb_place={"relx":0.37,"rely":0.65,"relwidth":0.188,"relheight":0.337}
        sc_place={"relx":0.91, "rely":0, "relwidth":0.088, "relheight":1}
        list_str=["用户"+str(iii) for iii in range(1,100)]
        self.createScrollbox(self.top,lb_place,sc_place,list_str,hellou)
    ...
    def gggg(self):
        lllll = Label(self.TabStrip1__Tab1, text="-"*101+
        "\n|　　文件名　　｜文件大小｜　      储存路径　      |　　 修改时间　 　｜      创建时间　 　｜\n"+"-"*101)
                
        lllll.place(relx=0.03, rely=0.005, relwidth=1, relheight=0.19)

        lb_place2={"relx":0.038,"rely":0.175,"relwidth":0.99,"relheight":0.7}
        sc_place2={"relx":0.936, "rely":0, "relwidth":0.038, "relheight":1}
        list_str2=["文件{0:<18}大小{1:<17}路径{2:<22}修改时间{3:<14}创建时间{4}".format(iii,iii,iii,iii,iii) for iii in range(1,22)]

        self.TabStrip1__Tab1Lbl = self.createScrollbox(self.TabStrip1__Tab1,lb_place2,sc_place2,list_str2,hellou)
        self.TabStrip1__lf = LabelFrame(self.TabStrip1__Tab1,text="")
        self.TabStrip1__lf.place(relx=0.2, rely=0.873, relwidth=0.6, relheight=0.12)
        self.TabStrip1__lb3 = Label(self.TabStrip1__Tab1,text="已选择文件：")
        self.TabStrip1__lb3.place(relx=0.035, rely=0.884, relwidth=0.16, relheight=0.12)
        self.TabStrip1__bt = Mybutton(master=self.TabStrip1__Tab1,fun=show_dw_filedialog,text="下载(D)")
        self.TabStrip1__bt.place(relx=0.85, rely=0.89, relwidth=0.12, relheight=0.1)
        


        
        tmp_list = ["用户编号", "文件编号", "操作类型"]
        self.TabStrip1__Tab2Lbl=Scrollabe_Frame(self.TabStrip1__Tab2,300,300)
        lllll = Label(self.TabStrip1__Tab2, text="-"*101+
        "\n| 用户编号｜文件编号｜操作类型｜       储存路径　       |　　 操作时间  　｜\n"+"-"*101)
        lllll.place(relx=0.031, rely=0.003, relwidth=1, relheight=0.19)
        for i in range(1, 9):
            for j in range(3):
                self.TabStrip1__Tab2Lbl = Label(
                    self.TabStrip1__Tab2, text=tmp_list[j]+str(i))
                self.TabStrip1__Tab2Lbl.place(
                    relx=0.05+0.2*j, rely=0.2+0.1*i, relwidth=0.15)
    ...
    def createSearch(self):
        e = Myentry(self.top)
        e.place(relx=0.368, rely=0.028, relwidth=0.19, relheight=0.048)
    ...
    def createRadio(self):
        ...
        v.set(1)
        radio_place={
            "显示所有":{"relx":0.23, "rely":0.028, "relwidth":0.068, "relheight":0.048},
            "模糊匹配":{"relx":0.287,"rely":0.028, "relwidth":0.068, "relheight":0.048}
        }
        ...
        r2.place(relx=0.287, rely=0.028, relwidth=0.068, relheight=0.048)
    ...
    def createScrollbox(self,master,listbox_place,scroll_place,list_str,list_fun):

        listbox=Listbox(master, selectmode=EXTENDED)
        scrollbar = Scrollbar(listbox, orient=VERTICAL)
        listbox.config(yscrollcommand=scrollbar.set)
        scrollbar.config(command=listbox.yview)
        listbox.place(**listbox_place)
        scrollbar.place(**scroll_place)
        for fs in list_str:
            listbox.insert(END, fs)
            listbox.bind("<Double-Button-1>", list_fun)
            listbox.select_set(list_str.index(list_str[0]))
            listbox.see(list_str.index(list_str[0]))
