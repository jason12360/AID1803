...
class MyGUI(Frame):
    ...
    def createNotebook(self):
        ...
        for t in range(len(T)):
            ...
            Tabs[t].title=Scrollabe_Frame(Tabs[t])
            Tabs[t].title.place(**self.dic["Tab_title"])
            Tabs[t].lbox=Scrollabe_Frame(Tabs[t])
...
class Application(MyGUI):
    ...
    def __init__(self, **kwargs):
        ...
        self.logoutlabel.config(text="hfgdhgdf导出日志导出日志导出日志导出日志导出日志导出日志导出日志导出日志导出日志导出日志.py")
    ...
    def logs_display(self,logs_list):
        widths=[7,7,7,17,40]
        title2=["用户编号","文件编号","操作类型","操作时间","储存路径"]
